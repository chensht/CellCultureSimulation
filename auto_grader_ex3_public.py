...
required_funcs = ["def transcribe(self, dna_seq)", "class MutantCell(StemCell)", "class CancerCell(MutantCell)",
                  "prosite_to_python(self, pattern_dict)", "patterns_to_domains(self, pattern_file)",
                  "def classify(self, seq_list, csv_file)", "class SequenceClassifier",
                  "def __init__(self, pattern_file)", "def __init__(self, genome, num_mutations=0)",
...
example_inputs = {os.path.join(os.path.dirname(sys.argv[0]), "ex3_inputs.json"): re.compile(
    "Original cell: <MutantCell, 2, 3>;+Final number of cells: (200|198);+Protein repertoire size: [0-9]+;+Mutations: 48;",
    re.MULTILINE), os.path.join(os.path.dirname(sys.argv[0]), "ex3_bad_inputs.json"): None}
...
def run_tests(python_command, file, curr_input, gold_result):
    ...
    # else if the expected output is AssertionError
        ...
        if [f for f in result_stderr.replace("'", "").replace(" ", "").replace("\t", "").split("\n") if f] and \
                [f for f in result_stderr.replace("'", "").replace(" ", "").replace("\t", "").split("\n") if f][
        ...
        if gold_result.fullmatch(";".join(result_stdout.split("\n"))):
            ...
            print("FAILED\n")
            print("\n".join(
                ["Expected Output:", gold_result.pattern.replace(";+", "\n"), "\nGiven Output:", result_stdout]))
...
if __name__ == '__main__':
    ...
    with open(full_submission_file) as f:
        ...
        [print("Class or function is not properly named, expected:", fun) for fun in required_funcs if
         not fun + ":" in f]
